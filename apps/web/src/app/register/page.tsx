  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`${SERVER_URL}/api/auth/register`, {
        ...
        body: JSON.stringify({ name, email, password, workspaceKey }),
      });
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-slate-500 mt-1">Creá tu cuenta de operador</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
          <form onSubmit={(e) => void handleSubmit(e)} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <label htmlFor="workspaceKey" className="text-sm font-medium text-slate-700">
                Workspace Key
              </label>
              <input
                id="workspaceKey"
                ...
                placeholder="dev_key_inboxchat_local"
                required
                className="px-3.5 py-2.5 rounded-lg border border-slate-300 text-sm text-slate-800 font-mono outline-none focus:border-slate-500 transition-colors"
              />
              <p className="text-xs text-slate-400">
                La key de tu workspace de InboxChat
              </p>
            </div>
